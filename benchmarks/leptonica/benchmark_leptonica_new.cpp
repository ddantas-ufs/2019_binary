...
";
	...
	{
		p++;
		img_bw = pixThresholdToBinary(img, thresh);
	}
	...
	invert_bw = pixInvert(NULL, img_bw);
	p = 0;
	TimerStart();
	while (p < nSteps)
	{
		p++;
		pixInvert(invert_bw, img_bw);
	}
	printf("Time spent on %8d         Not:           %s\n", nSteps, getTimeElapsedInSeconds());
	
	saveResult(invert_bw, outPath, "invert_bw");


	// Max
	// https://github.com/DanBloomberg/leptonica/blob/master/src/pix3.c
	result_bw = pixOr(NULL, mask_bw, img_bw);
	p = 0;
	TimerStart();
	while (p < nSteps)
	{
		p++;
		pixOr(result_bw, mask_bw, img_bw);        
	}
	printf("Time spent on %8d         Or:           %s\n", nSteps, getTimeElapsedInSeconds());
	...
	printf("Time spent on %8d         Unpack:           %s\n", nSteps, getTimeElapsedInSeconds());
  ...
  // selCreateBrick(), with input (h, w, cy, cx, val)
	...
	// https://github.com/DanBloomberg/leptonica/blob/master/src/morphdwa.c
	...
	pixFMorphopGen_1(result_border_bw0, input_border_bw, L_MORPH_DILATE, (char*)"sel_3h");
	pixFMorphopGen_1(result_border_bw, result_border_bw0, L_MORPH_DILATE, (char*)"sel_3v");
